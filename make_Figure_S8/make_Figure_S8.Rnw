...
for(file in 1:length(allFiles))
{
    ...
    {
      VarsAll[rowNr, colnames(Vars)] <- Vars[rowNr, colnames(Vars)]
    }
}
...
allFiles <- list.files("simResultsComb")
##keep only files with "Suppl_Bayes" in title
allFiles <- allFiles[grep("Suppl_Bayes", allFiles)]
...
for(file in 1:length(allFiles))
{
    ...
    rowNr <- gsub("combine_cost_of_estimation_Suppl_Bayes", "", allFiles[file])
    rowNr <- gsub(".RData", "", rowNr)
    rowNr <- as.numeric(rowNr)
...
<<sepPanelsAB, include=TRUE>>=
Vars.b <- VarsAll

##add in the 0 heterogeneity case
Vars.b <- rbind(Vars.b, Vars0)
...
Panel a):
<<Figure_S8_panel_a, fig.width=6, fig.height=5, dpi=800>>=
panelA <- ggplot(RelEff.c, aes(x=p, y=RelEff))+
...
Panel b):
<<Figure_S8_panel_b, fig.width=6, fig.height=5, dpi=800>>=
panelB <- ggplot(RelEff.c, aes(x=p, y=MVMAcomp))+
  geom_point(size=3.0, aes(color=as.factor(varBtw), shape=as.factor(varBtw)))+
  geom_line(aes(color=as.factor(varBtw), shape=as.factor(varBtw)))+
  themeUsed+
  ylab(expression(paste(Var, "(", mu[1]^M, ")", "/",
                        Var, "(", mu[1]^MB, ")"))) +
  ylim(min(RelEff.c$MVMAcomp)*0.8, 1.2) +
  labs(color="", shape="",
...
allFiles <- list.files("simResultsComb")
##remove files with "Suppl_Bayes" in title
allFiles <- allFiles[-grep("Suppl_Bayes", allFiles)]
...
Panel c):
<<Figure_2_panel_c, fig.width=6, fig.height=5, dpi=800>>=
panelC <- ggplot(RelEff.c, aes(x=p, y=RelEff))+
  ...
  themeUsed+
  ylab(expression(RelEff)) +
  ylim(min(RelEff.c$RelEff)*0.8, 1) +
  labs(color="", shape="", 
...
<<Figure_S8_panels_abcd, fig.width=12, fig.height=12, dpi=800>>=

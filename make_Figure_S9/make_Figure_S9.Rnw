...
allFiles <- list.files("simResultsComb")
##keep only files that have "bd_het" in the title
allFiles <- allFiles[grep("bd_het", allFiles)]
...
for(file in 1:length(allFiles))
{
    ...
    rowNr <- gsub("combine_cost_of_estimation_bd_het_", "", allFiles[file])
    rowNr <- gsub(".RData", "", rowNr)
    rowNr <- as.numeric(rowNr)
...
panelB <- ggplot(RelEff.c, aes(x=p, y=RelEff))+
...
Panel c):
<<Figure_S8_panel_c, fig.width=6, fig.height=5, dpi=800>>=
panelC <- ggplot(RelEff.c, aes(x=p, y=MVMAcomp))+
  geom_point(size=3.0, aes(color=I, shape=I))+
...
multiplot(panelA, panelC, panelB, cols=2)
@

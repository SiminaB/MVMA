...
\hlcom{##save results in data frame}
\hlstd{bigMat} \hlkwb{<-} \hlkwd{expand.grid}\hlstd{(}\hlkwc{rho} \hlstd{=} \hlkwd{c}\hlstd{(}\hlnum{0}\hlstd{,} \hlnum{0.25}\hlstd{,} \hlnum{0.5}\hlstd{,} \hlnum{0.75}\hlstd{),}
                      \hlkwc{p} \hlstd{=} \hlnum{2}\hlopt{:}\hlnum{1000}\hlstd{,}
                      \hlkwc{RelEff} \hlstd{=} \hlnum{NA}\hlstd{)}
...
Compare minimum values with known asymptotic limits:

\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{min}\hlstd{(bigMat}\hlopt{$}\hlstd{RelEff[bigMat}\hlopt{$}\hlstd{rho}\hlopt{==}\hlstr{"rho=0.25"}\hlstd{])}
\end{alltt}
\begin{verbatim}
## [1] 0.8901406
\end{verbatim}
\begin{alltt}
\hlkwd{min}\hlstd{(bigMat}\hlopt{$}\hlstd{RelEff[bigMat}\hlopt{$}\hlstd{rho}\hlopt{==}\hlstr{"rho=0.5"}\hlstd{])}
\end{alltt}
\begin{verbatim}
## [1] 0.7512425
\end{verbatim}
\begin{alltt}
\hlkwd{min}\hlstd{(bigMat}\hlopt{$}\hlstd{RelEff[bigMat}\hlopt{$}\hlstd{rho}\hlopt{==}\hlstr{"rho=0.75"}\hlstd{])}
\end{alltt}
\begin{verbatim}
## [1] 0.5808737
\end{verbatim}
\begin{alltt}
\hlstd{asympLimit} \hlkwb{<-} \hlkwa{function}\hlstd{(}\hlkwc{rho}\hlstd{,} \hlkwc{I}\hlstd{)}
\hlstd{\{}
  \hlstd{i} \hlkwb{<-} \hlnum{1}\hlopt{:}\hlstd{I}
  \hlstd{rho.i} \hlkwb{<-} \hlstd{rho}\hlopt{*}\hlstd{(i}\hlopt{-}\hlnum{1}\hlstd{)}\hlopt{/}\hlstd{I}

  \hlstd{I}\hlopt{*}\hlstd{(}\hlnum{1}\hlopt{/}\hlkwd{sum}\hlstd{(}\hlnum{1}\hlopt{/}\hlstd{rho.i)}\hlopt{+}\hlnum{1}\hlopt{/}\hlkwd{sum}\hlstd{(}\hlnum{1}\hlopt{/}\hlstd{(}\hlnum{1}\hlopt{-}\hlstd{rho.i)))}
\hlstd{\}}

\hlkwd{asympLimit}\hlstd{(}\hlnum{0.25}\hlstd{, I)}
\end{alltt}
\begin{verbatim}
## [1] 0.875323
\end{verbatim}
\begin{alltt}
\hlkwd{asympLimit}\hlstd{(}\hlnum{0.5}\hlstd{, I)}
\end{alltt}
\begin{verbatim}
## [1] 0.7344264
\end{verbatim}
\begin{alltt}
\hlkwd{asympLimit}\hlstd{(}\hlnum{0.75}\hlstd{, I)}
\end{alltt}
\begin{verbatim}
## [1] 0.5631485
\end{verbatim}
\begin{alltt}
\hlcom{##only plot the subset with p between 2 and 20}
\hlstd{bigMat} \hlkwb{<-} \hlstd{bigMat[bigMat}\hlopt{$}\hlstd{p} \hlopt{<=} \hlnum{20}\hlstd{,]}
\end{alltt}
\end{kframe}
\end{knitrout}

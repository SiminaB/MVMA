...
MSEs <- expand.grid(I = c(5, 10, 15, 20),
                    p = 2:10,
                    corrBtw = c(0, 0.5),
                    varBtw = c(0, 1))

##only keep combinations of corrBtw=0 & varBtw=0, corrBtw=0.5 & varBtw=0.5
MSEs <- MSEs[(MSEs$corrBtw == 0 & MSEs$varBtw == 0) |
             (MSEs$corrBtw == 0.5 & MSEs$varBtw == 1) ,]
...
##added on June 30, 2016: also save Bayesian estimates
...
save(list=c("MSEs", "Vars",
     ...
     "OverallEstSigmaUniv"),
     file=paste("simResultsComb/combine_cost_of_estimation_",rowNr,".RData", sep=""))
